...
class support{
    ...
    function support_all($db){
        ...
        foreach($list as $item){
            ...
            foreach((array)$sub_list as $sub_item){
                ...
                $sub_lnk=myWeb.$this->lang.'/'.$this->view.'/'.common::slug($title).'-i'.$sub_item['id'].'.html';
                $str.='
    ...
    function support_cate($db,$pId=0){
        ...
        $str.='
            ...
            <div class="row">
                ...
                <p style="word-wrap: break-word;">
                    <ul class="hotroitem">';
        $db->where('pId',$pId)->where('active',1)->orderBy('ind','ASC')->orderBy('id');
    ...
    function support_one($db,$id=0){
        $db->where('id',$id);
        $item=$db->getOne('sp','id,title,e_title,content,e_content,pId');
        if($this->lang=='en'){
            $title=$item['e_title'];
        ...
        $str.='
            ...
            <div class="col-md-9">
                ...
                <h3>'.other.'</h3>
                <hr/>';
        $db->where('active',1)->where('pId',$item['pId'])->where('id',$item['id'],'<>')->orderBy('ind','ASC')->orderBy('id');
        $db->pageLimit=10;
        $list=$db->paginate('sp',1,'id,title');
        $str.='
        <p style="word-wrap: break-word;">
                    <ul class="hotroitem">';
        foreach($list as $item){
